...
type yunjiasuCert struct {
	...
	SyncToNamespaces []string  `json:"sync_to_namespaces"`
}
...
func (y *yunjiasu) CheckCerts() {
	...
	for _, v := range y.certs {
		go func(cert yunjiasuCert) {
			...
			for _, ns := range cert.SyncToNamespaces {
				defaultCert, err := y.k8sClientset.CoreV1().Secrets(ns).Get(cert.TlsName, metav1.GetOptions{})
				if err != nil || defaultCert == nil {
					...
					newCert.Data["tls.crt"] = cert.TlsCrt
					newCert.Data["tls.key"] = cert.TlsKey
					_, err = y.k8sClientset.CoreV1().Secrets(ns).Create(newCert)
					if err != nil {
						klog.Errorf("[SyncToNamespaces] Create:%s Namespace:%s error: %s", cert.Domain, ns, err.Error())
						continue
					}
				}
			}
			...
			if cert.ExpiresOn.Before(cert.TlsNotAfter) {
				...
				for _, ns := range cert.SyncToNamespaces {
					defaultCert, err := y.k8sClientset.CoreV1().Secrets(ns).Get(cert.TlsName, metav1.GetOptions{})
					if defaultCert != nil {
						defaultCert.Data["ca.crt"] = []byte("")
						defaultCert.Data["tls.crt"] = cert.TlsCrt
						defaultCert.Data["tls.key"] = cert.TlsKey
						_, err = y.k8sClientset.CoreV1().Secrets(ns).Update(defaultCert)
						if err != nil {
							klog.Errorf("[SyncToNamespaces] Update:%s Namespace:%s error: %s", cert.Domain, ns, err.Error())
							continue
						}
					}
				}
	...
	for _, v := range y.cfg.Certs {
		go func(cert config.CertConfig) {
			...
			cr := yunjiasuCert{
				...
				SyncToNamespaces: cert.SyncToNamespaces,
			}
			...
			for i := 0; i < y.cfg.Common.SyncRetryTimes; i++ {
				...
				klog.Error("[SyncYunjiasuCerts] retry after 10 seconds")
